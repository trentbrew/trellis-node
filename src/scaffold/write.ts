...
function writeTrellisHooks(rootPath: string, ide: IdeType): void {
  ...
  const adapterName =
    ide === 'windsurf' ? 'cascade-adapter.sh' : `${ide}-adapter.sh`;
  writeFileSync(
  ...
  switch (ide) {
    ...
    case 'windsurf':
      writeFileSync(
        join(rootPath, '.windsurf', 'hooks.json'),
...
function renderWindsurfRules(input: IdeScaffoldInput): string {
  const projectName = input.context.name ?? 'this project';
  const domain = input.context.domain ?? 'general';
  const eco = input.context.ecosystem ?? 'unknown';
  const framework =
...
function renderWindsurfHooksConfig(): Record<string, unknown> {
  return {
    hooks: {
      pre_user_prompt: [
        {
      ...
      post_cascade_response: [
        {
          command:
            'bash .cursor/hooks/adapters/cascade-adapter.sh post-response',
          show_output: true,
        },
      ],
    },
          ...
          hooks: [
            {
              ...
              timeout: 15000,
              description:
                'Mirror Windsurf pre_user_prompt desk context refresh',
            },
          ],
        },
      ],
...
export function writeIdeScaffold(
  ...
  switch (ide) {
    ...
    case 'windsurf': {
      const windsurfDir = join(rootPath, '.windsurf');
      if (!existsSync(windsurfDir)) {
        mkdirSync(windsurfDir, { recursive: true });
      }
      writeFileSync(
        join(windsurfDir, 'rules.md'),
